...
contract FullCoverage is DSTest {
    ...
    function Gnosis()public returns(GnosisSafe){
    ...
    function testCreateProposal()public{
        ...
        cheats.prank(cheats.addr(1));
        module.createProposal(command1);
        assert(module.getProposalCount() > 0);
    }
    ...
    function testExecuteRemoveOwner()public{
        ...
        cheats.prank(cheats.addr(3)); 
        module.vote(0);// Address 3 votes to approve the proposal 
        cheats.warp(1 weeks);
        cheats.prank(cheats.addr(1));
        module.executeProposal(0);// Address 1 executes proposal
        assert(!safe.isOwner(cheats.addr(5))); // Check address 5 is no longer owner.
        
        
    }

    function testFailThresholdNotReached()public{
        ConfigurableRightsPool pool = Balancer();
        module = new VoterModule(address(safe),address(safe),address(safe),address(pool));
        GnosisSafe testSafe = Gnosis();
        uint[] memory num = new uint[](2);
        ...
        module.createProposal(command2); // Add proposal to remove address 5 as an owner 
        cheats.prank(cheats.addr(1)); 
        module.vote(0);// Address 1 votes for the proposal but with out enought weight to execute
        cheats.warp(1 weeks);
        cheats.prank(cheats.addr(1));
        module.executeProposal(0);// Address 1 executes proposal
        assert(!safe.isOwner(cheats.addr(5))); // Check address 5 is no longer owner.
    ...
    function testFailExecutionWindow()public{
        ConfigurableRightsPool pool = Balancer();
        module = new VoterModule(address(safe),address(safe),address(safe),address(pool));
        GnosisSafe testSafe = Gnosis();
        uint[] memory num = new uint[](2);
        ...
        cheats.prank(cheats.addr(1)); 
        module.vote(0);// Address 3 votes to approve the proposal 
        cheats.warp(3 weeks); // More than 2 weeks have passed
        cheats.prank(cheats.addr(1));
        module.executeProposal(0);// Address 1 executes proposal
        assert(!safe.isOwner(cheats.addr(5))); // Check address 5 is no longer owner.
